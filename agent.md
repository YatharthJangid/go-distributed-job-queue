...
> Comprehensive operational guide, architecture reference, and system manual for AI agents and engineers working on the **GoRes** distributed job queue repository.

---

## 1. Project Overview & Mission

**GoRes** is a production-grade, Redis-backed distributed background job queue written in Go. It is engineered for ultra-high throughput, reliability, zero-allocation hot paths, and robust multi-node distributed coordination.

### Core Metrics & Capabilities
* **Peak Throughput**: >35,000 jobs/sec in live benchmarks; >100,000 jobs/sec in batch enqueue.
* **Dispatch Latency**: ~845 ns per job deserialization/dispatch.
* **Zero-Allocation Hot Path**: Backed by `sync.Pool` recycling `Job` structs.
* **Reliable At-Least-Once Delivery**: Redis atomic `BRPOPLPUSH` ensures zero job loss during worker crashes.
* **Cluster-Wide Idempotency**: Atomic Lua deduplication at enqueue and distributed execution status keys at worker runtime.
* **Multi-Node Kubernetes Ready**: Full manifest suite for multi-replica, multi-node Kubernetes deployments with graceful termination.
...
│   ├── config.go                 # Config struct + JSON loader + REDIS_URL/ENV overrides
│   ├── job.go                    # Job struct, sync.Pool allocator, MessagePack codec
│   ├── gores.go                  # Gores client, connection pool, Lua enqueue, Info & status
│   ├── worker.go                 # Worker pool, BRPOPLPUSH loop, retry, DLQ, execution lock
...
│
└── graphify-out/                 # Graphify Code Knowledge Graph & Visualizer
    ├── graph.json                # GraphRAG-ready JSON knowledge graph
    ├── graph.html                # Standalone interactive browser visualization
    ├── GRAPH_REPORT.md           # Graph audit report, God nodes, and community analysis
    └── manifest.json             # File dependency and extraction manifest
```
...
│   → Atomic Lua script with SET NX EX deduplication          │
└──────────────────────────────┬──────────────────────────────┘
...
│   → Check / acquire gores:exec:<key> execution lock         │
...
* **Reliable Dequeue (`BRPOPLPUSH`)**: Atomically shifts job bytes from `:pending` to `:processing`. If a worker crashes mid-task, the job remains in `:processing` and is never lost.
...
GoRes provides end-to-end idempotency guarantees across two distinct layers:
...
### Layer 2: Execution Deduplication
* **Purpose**: Prevents double-processing when messages are re-delivered or distributed across multiple worker nodes.
* **Mechanism**:
...
GoRes includes production-ready Kubernetes manifests in [`k8s/`](file:///home/jangi/projects/gores/k8s).
...
## 8. Knowledge Graph Maintenance (`/graphify`)

GoRes uses **graphify** to maintain an up-to-date, queryable knowledge graph of the codebase.
...
When editing or extending this codebase, adhere strictly to the following principles:

1. **Preserve Package Structure**: All core queue logic belongs in `pkg/gores` under `package gores`. Avoid creating generic helper packages (`lib`, `util`, `common`).
2. **Zero Allocations on Hot Paths**: When modifying `Job` creation or parsing, always acquire via `GetJob()` and release via `PutJob(j)` in `defer`.
3. **Preserve Atomicity**: Never replace Redis Lua scripts with multi-step non-atomic roundtrips for queue operations.
4. **Maintain Redis Compatibility**: Support both explicit host/port config (`config.json`) and URL-based cloud config (`REDIS_URL`).
5. **Always Update Tests & Graph**: Whenever new features are added, add unit tests in `pkg/gores/*_test.go` and run `graphify update .` to keep the knowledge graph synchronized.
